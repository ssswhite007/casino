...
<h5 class="text-2xl font-medium text-white mb-[25px]">{$LL.HEY_HELLO()}</h5>
		...
		{:else if registrationStep == 2}
			...
			<div class="flex flex-col gap-[9px]">
				<label class="text-msm text-white font-medium" for="promotionCode"
					>{$LL.PROMOTION_CODE_OPTIONAL()}</label
				>
				<div class="w-full flex">
					<input
						type="promotionCode"
		...
		{:else if registrationStep == 3}
			<div class="flex items-center justify-between mt-[15px] gap-[4px]">
				<p class="text-msm text-grayDark3 font-medium">
					{$LL.SUCCESSFULLY_SIGNUP()}
					<a href="/" on:click={openSignIn} class="gradient-text font-bold">{$LL.SIGN_IN()}</a>
				</p>
			</div>
		{/if}
		<button
